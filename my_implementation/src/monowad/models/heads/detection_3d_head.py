"""Anchor-based 3D detection head for MonoWAD.

Ported from visualDet3D/networks/heads/detection_3d_head.py. Changes vs. legacy:

- Relative imports into ``monowad`` (anchors, losses, blocks, dcn, utils.geometry);
  the modulated deformable conv now comes from the torchvision-backed ``..dcn``.
- Device-safe: the one hard-coded ``.cuda()`` (zero cls-loss for empty annotations)
  is replaced with a device-following ``new_zeros``.
- ``post_optimization`` (the legacy hill-climbing ``_post_process`` using the un-ported
  ``iou3d`` CUDA op) is not carried over; enabling it raises a clear error.

Config is passed as mapping-like objects (plain dict / OmegaConf DictConfig); both
support ``**`` unpacking and ``getattr(cfg, key, default)``.
"""
...
from ..blocks import AnchorFlatten
from ..dcn import ModulatedDeformConvPack
from ..losses import ModifiedSmoothL1Loss, SigmoidFocalLoss
from ...utils.geometry import BackProjection, ClipBoxes, calc_iou
...
class AnchorBasedDetection3DHead(nn.Module):
    def __init__(
        ...
        super().__init__()
        anchors_cfg = anchors_cfg or {}
        layer_cfg = dict(layer_cfg or {})
        loss_cfg = loss_cfg or {}
        test_cfg = test_cfg or {}

        self.anchors = Anchors(
            preprocessed_path=preprocessed_path, readConfigFile=read_precompute_anchor, **anchors_cfg
        )
        ...
        if layer_cfg.get("num_anchors", None) is None:
    ...
    def build_loss(self, focal_loss_gamma=0.0, balance_weight=(0,), L1_regression_alpha=9, **kwargs):
        ...
        regression_weight = kwargs.get(
            "regression_weight", [1 for _ in range(self.num_regression_loss_terms)]
        )  # default 12, only used in 3D
    ...
    def _assign(
        ...
        **kwargs,
        ...
        """anchor: [N, 4]; annotation: [num_gt, >=5] (xyxy + label)."""
        N = anchor.shape[0]
        num_gt = annotation.shape[0]
        assigned_gt_inds = anchor.new_full((N,), -1, dtype=torch.long)
        max_overlaps = anchor.new_zeros((N,))
        assigned_labels = anchor.new_full((N,), -1, dtype=torch.long)
    ...
    def _decode(self, boxes, deltas, anchors_3d_mean_std, label_index, alpha_score):
        ...
        pred_z = deltas[..., 6] * selected_mean_std[:, 0, 1] + selected_mean_std[:, 0, 0]
    ...
    def _sample(self, assignment_result, anchors, gt_bboxes):
        """Pseudo sampling."""
        pos_inds = (
            torch.nonzero(assignment_result["assigned_gt_inds"] > 0, as_tuple=False).unsqueeze(-1).unique()
        )
        neg_inds = (
            torch.nonzero(assignment_result["assigned_gt_inds"] == 0, as_tuple=False).unsqueeze(-1).unique()
        )
    ...
    def _get_anchor_3d(self, anchors, anchor_mean_std_3d, assigned_labels):
        """Select per-positive-anchor 3D priors for the assigned class."""
        ...
        selected_mask = selected_anchor_3d[:, 0, 0] > 0  # only z > 0 (valid prior)
    ...
    def get_bboxes(self, cls_scores, reg_preds, anchors, P2s, img_batch=None):
        ...
        cls_agnostic = getattr(self.test_cfg, "cls_agnositc", True)  # True -> direct NMS
        ...
        if cls_agnostic:
            ...
            nms_bbox = bboxes[:, :4] + label.float().unsqueeze(-1) * (max_coordinate)
        ...
        if getattr(self.test_cfg, "post_optimization", False):
            raise NotImplementedError(
                "post_optimization (hill-climbing/iou3d) was not ported; keep post_optimization=False."
            )
    ...
    def loss(self, cls_scores, reg_preds, anchors, annotations, P2s):
        ...
        for j in range(batch_size):
            ...
            if len(bbox_annotation) == 0:
                cls_loss.append(cls_score.new_zeros(()))
            ...
            labels = anchor_j.new_full((num_valid_anchors, self.num_classes), -1, dtype=torch.float)
            ...
            if len(pos_inds) > 0:
                ...
                if len(selected_anchor_3d) > 0:
                    ...
                    if self.decode_before_loss:
                        ...
                        pos_prediction_decoded = self._decode(
                            pos_anchor, reg_pred[pos_inds], anchor_mean_std_3d_j[pos_inds], label_index, pos_alpha_score
                        )
                        pos_target_decoded = self._decode(
                            pos_anchor, pos_bbox_targets, anchor_mean_std_3d_j[pos_inds], label_index, pos_alpha_score
                        )
