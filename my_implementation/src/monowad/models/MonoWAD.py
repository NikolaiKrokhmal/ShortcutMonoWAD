...
from .backbone.dla import dla102
from .backbone.dlaup import DLAUp
from .dfe import DepthAwareFE
from .dpe import DepthAwarePosEnc
from .dtr import DepthAwareTransformer
from .diffusion.denoising_diffusion_pytorch import Unet, GaussianDiffusion
from .wc import WeatherCodebook
            

class MonoWAD(nn.Module):
    def __init__(self, backbone_arguments=dict()):
        super(MonoWAD, self).__init__()
        # `pretrained` controls the one-time ImageNet DLA download. Set it False in the
        # config when loading a full MonoWAD checkpoint (the weights are overwritten
        # anyway) to avoid any network access. `flash_attn` toggles the SDPA flash path
        # in the diffusion U-Net (needs a GPU). Both default to legacy behaviour.
        self.pretrained = bool(backbone_arguments.get("pretrained", True))
        self.flash_attn = bool(backbone_arguments.get("flash_attn", True))
        self.backbone = dla102(pretrained=self.pretrained, return_levels=True)
