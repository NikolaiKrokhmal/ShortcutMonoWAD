...
def get_split_parts(num, num_part):
    # Deviation from the verbatim visualDet3D copy: when num < num_part the original
    # returns [0]*num_part + [num], i.e. leading zero-size partitions that make the
    # downstream np.concatenate([]) crash. The legacy code only ever ran the full val
    # set (num >> num_part) so never hit it; we guard so partial / smoke-test val runs
    # score fine too.
    if num < num_part:
